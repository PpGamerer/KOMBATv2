...
export default function PlaceMinion() {
    ...
    useEffect(() => {
        ...
        const checkAndTriggerBot = async () => {
            try {
                ...
                const actualCurrentPlayer = currentPlayerName?.includes("1") ? 1 : 2;
                ...
                if (shouldTriggerBot && !isBotPlaying) {
                    ...
                    const delay = isFreeDrop ? 1500 : 2000;
        ...
        const timeoutId = setTimeout(checkAndTriggerBot, 300);
        return () => clearTimeout(timeoutId);
    }, [currentPlayer, gameInitialized, isBotPlaying, isPlayer1Bot, isPlayer2Bot, isFreeDrop]);
    ...
    const checkWinner = (gameState: any) => {
        ...
        const winnerLog = logs.find((log: string) => log.includes("Game Over! Winner:"));

        if (winnerLog) {
            const winnerName = winnerLog.split("Winner:")[1]?.trim();
            ...
            if (player1.ownedHexCount > player2.ownedHexCount) {
                setWinner(player1.name || "Player 1");
            } else if (player2.ownedHexCount > player1.ownedHexCount) {
                setWinner(player2.name || "Player 2");
            } else {
                setWinner("Draw");
            }
        }
    ...
    }, [showStartScreen, isFreeDrop, currentPlayer, gameInitialized, isPlayer1Bot, isPlayer2Bot]);
    ...
    const handleNoPlace = async () => {
        ...
        try {
            ...
            if (response.gameOver) {
                setIsGameOver(true);
                checkWinner(response.gameState);
                return;
            }
        ...
        try {
            ...
            if (freeSpawnCompleted) {
                ...
                setTimeout(() => {
                    ...
                    // Only show prompt if next player is human
                    if ((nextPlayer === 1 && !isPlayer1Bot) || (nextPlayer === 2 && !isPlayer2Bot)) {
                        askTurn(nextPlayer);
                    }
                    // If bot, useEffect will trigger automatically
                }, 1000);
            }
        } catch (error) {
    ...
    const placeFreeDropMinion = async (id: number) => {
        ...
        try {
            ...
            if (freeSpawnCompleted) {
                ...
                if (!shouldShowPrompt(nextPlayerNum)) {
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-lg text-center w-[400px]">
                        ...
                        <button onClick={() => setMessage(null)}
                                className="bg-blue-500 text-white px-6 py-2 rounded">OK
                        </button>
                    </div>
                </div>
            )}
                    ...
                    {isGameOver && (
                        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
                            <div className="bg-white p-8 rounded-lg text-center space-y-4">
                                <div className="text-4xl font-bold text-black mb-4">🏆 Game Over! 🏆</div>
                                ...
                                {!winner && (
                                    <div className="text-xl text-gray-700 mb-4">Max turns reached!</div>
                                )}
                                ...
                                <div className="text-xl font-bold text-black mt-6">
                                    ...
                                    <div className="flex justify-center space-x-6 mt-4">
                                        <button onClick={handleNo}
                                                className="bg-green-500 text-white px-6 py-3 rounded-lg text-lg hover:bg-green-600">Yes
                                        </button>
                                        <button onClick={handleRestart}
                                                className="bg-red-500 text-white px-6 py-3 rounded-lg text-lg hover:bg-red-600">No
                                        </button>
                                    </div>
                                </div>
                            </div>
